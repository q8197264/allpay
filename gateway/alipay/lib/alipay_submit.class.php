...
class AlipaySubmit {
    ...
    function buildRequestUrl($parameter)
    {
        ...
        //对待签名参数数组排序
        $para_sort = argSort($parameter);

        //把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串，并对字符串做urlencode编码
        $urlpara = createLinkstringUrlencode($para_sort);

        //生成签名结果
        $sign = $this->buildRequestMysign($para_sort);echo '<p>';
        $sign_type = strtoupper(trim($this->alipay_config['sign_type']));

        $url = $this->alipay_gateway_new."{$urlpara}&sign={$sign}&sign_type={$sign_type}";
