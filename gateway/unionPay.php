 ...
 * Time: 15:38
 */
class unionPay extends Gateway
{
    /**
     * TODO:支付宝 银联支付
     * @param int $order_id
    ...
    function pay( $data )
    {
        ...
        if ( empty($data['bankcode']) ){
            //银联支付
            throw new AllPayException('bank code is empty :'.$data['bankcode'] );
        }
        if ( !empty($data['out_trade_no']) && is_array($data) )
        {
            if ( is_file(__DIR__.'/alipay/AlipayAdaptor.php') ){
                ...
                CY_log::add("$msg <<<".json_encode($data));
    ...
    function notify()
    {
        ...
        if ( is_file(__DIR__.'/alipay/AlipayAdaptor.php') )
        {
            ...
            CY_log::add( $alipayAdaptor );
            if( $alipayAdaptor->verifyNotify() )//验证成功
            {
                //支付宝交易号
                ...
                if( $trade_status == 'TRADE_FINISHED' OR $trade_status == 'TRADE_SUCCESS') {
                    ...
                    CY_log::add('alipaybankNotify:验证通过');
                    return $result;
                }else{
                    //验证失败
                    $msg = 'alipaybankNotify:交易失败';
                }
            }else {
                //验证失败
                $msg = 'alipaybankNotify:验证失败';
            }
        }else{
        ...
        if ( is_file(__DIR__.'/alipay/AlipayAdaptor.php') ) {
            ...
            {//验证成功
                ...
                if($trade_status == 'TRADE_FINISHED' || $trade_status == 'TRADE_SUCCESS') {
                    ...
                    CY_log::add('alipaybankReturn:验证通过');
                    return $result;
                }
            }else{
                //验证失败
                ...
                throw new AllPayException('alipaybankReturn:验证失败');
            }
