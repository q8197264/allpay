 ...
 *
 */
if (function_exists("date_default_timezone_set")) date_default_timezone_set(quickpay_conf::$timezone);
class quickpay_service
{
    ...
    function quickpay_service($args, $service)
    {
        ...
        switch($service)
        {
            case quickpay_conf::FRONT_PAY:
                ...
                if ($trans_type != quickpay_conf::CONSUME && $trans_type != quickpay_conf::PRE_AUTH) {
                    //前台交易仅支持 消费 和 预授权
                    throw new AllPayException("Bad trans_type for front_pay. Use back_pay instead");
                }*/
                $this->api_url = quickpay_conf::$front_pay_url;
                $this->args = array_merge(quickpay_conf::$auth_params, $args);
                $param_check = quickpay_conf::$auth_params_check;
            ...
            case quickpay_conf::BACK_PAY:
                ...
                if ($trans_type == quickpay_conf::CONSUME || $trans_type == quickpay_conf::PRE_AUTH) {
                    if (!isset($this->args['cardNumber']) && !isset($this->args['pan'])) {
                        throw new AllPayException('consume OR pre_auth transactions need cardNumber!');
                    }
                }
                else {
                    if (empty($this->args['origQid'])) {
                        throw new AllPayException('origQid is not provided');
                    }
                }
                break;
            case quickpay_conf::QUERY:
                $this->api_url		= quickpay_conf::$card_query_url;
                ...
                {
                    throw new AllPayException('merId and acqCode can\'t be both empty');
                }
                ...
                $this->args = $args;
                $param_check = quickpay_conf::$query_params_check2;
                break;
            case quickpay_conf::PAY_QUERY:
                ...
                $param_check = quickpay_conf::$query_params_check2;
                break;
            case quickpay_conf::SMS_SEND:
                $this->api_url		= quickpay_conf::$sms_query_url;
                $args['version']    = quickpay_conf::$pay_params['version'];
                $args['charset']    = quickpay_conf::$pay_params['charset'];
                $args['merId']      = quickpay_conf::$pay_params['merId'];
                ...
                $this->args = $args;
                $param_check = quickpay_conf::$query_params_check3;
                break;
            case quickpay_conf::RESPONSE:
                ...
                if (!isset($arr_args['signature']) || !isset($arr_args['signMethod'])) {
                    throw new AllPayException('No signature Or signMethod set in notify data!');
                }
                ...
                if ($signature != $this->signature) {
                    throw new AllPayException('Bad signature returned!');
                }
            ...
            default:
                throw new AllPayException("Unknown service provided.");
        }
        ...
        foreach ($param_check as $key) {
            if (!isset($this->args[$key])) {
                throw new AllPayException("KEY [$key] not set in params given");
            }
        }
    ...
    static function encode($value, $output_charset, $input_charset)
    {
        ...
        throw new AllPayException("sorry, mbstring or iconv is needed for charset conversion.");
    }
    ...
    static function sign($params, $sign_method)
    {
        ...
        /* TODO: elseif (strtolower($sign_method) == "rsa")  */
            ...
            throw new AllPayException("Unknown sign_method set in quickpay_conf");
        }
    }
...
eot;
    ...
    static function curl_call($url, $data = null, $is_post = true, $options = null)
    {
        ...
        /* TODO: elseif (function_exists('fsockopen')) { } */
            ...
            throw new AllPayException("[PHP] curl module is required");
        }
    }
}
